import sys


def count_GC(fastq_read):
    count = 0
    for base in fastq_read:
        if base == 'G' or base == 'C':
            count += 1
    return int(count * 100 / len(fastq_read))


def write_read_to_file(fastq_read_lines, file):
    file.write('\n'.join(fastq_read_lines) + '\n')


def validate_read(fastq_read, min_length, gc_bounds):
    if len(fastq_read) < min_length:
        return False
    if len(gc_bounds) == 1:
        if count_GC(fastq_read) < gc_bounds[0]:
            return False
    if len(gc_bounds) == 2:
        isLess = count_GC(fastq_read) < gc_bounds[0]
        isMore = count_GC(fastq_read) > gc_bounds[1]
        if isLess or isMore:
            return False
    return True

def filtrate(reads, min_len, gc):
    passed_reads = []
    failed_reads = []
    for i in range(0, len(reads), 4):
        fastq_read_info = reads[i:i + 4]
        
        if validate_read(fastq_read_info[1], min_len, gc):
            passed_reads.append(fastq_read_info)
        else:
            failed_reads.append(fastq_read_info)
    return passed_reads, failed_reads


# untested. just read params and call functions logic
def do():
    last_arg = sys.argv[len(sys.argv) - 1]
    splitted_by_dots = last_arg.split('.')

    MIN_LENGTH_ARG = '--min_length'
    KEEP_FILTERED_ARG = '--keep_filtered'
    GC_BOUNDS_ARG = '--gc_bounds'
    OUTPUT_BASE_NAME_ARG = '--output_base_name'

    valid_args = [
        MIN_LENGTH_ARG,
        KEEP_FILTERED_ARG,
        GC_BOUNDS_ARG,
        OUTPUT_BASE_NAME_ARG
    ]

    min_length = 0
    keep_filtered = False
    gc_bounds = []
    input_file = sys.argv[-1]

    output_base_name = '.'.join(splitted_by_dots[0:len(splitted_by_dots) - 1])

    i = 1
    while i < len(sys.argv) - 1:
        if sys.argv[i] not in valid_args:
            raise ValueError('Invalid argument: ' + sys.argv[i])
        if sys.argv[i] == MIN_LENGTH_ARG:
            min_length = int(sys.argv[i + 1])
            if min_length <= 0:
                raise ValueError(MIN_LENGTH_ARG + ' must be greater than 0')
            i += 2
            continue
        if sys.argv[i] == KEEP_FILTERED_ARG:
            keep_filtered = True
            i += 1
            continue
        if sys.argv[i] == GC_BOUNDS_ARG:
            i += 1
            while sys.argv[i].isnumeric():
                gc_bounds.append(int(sys.argv[i]))
                i += 1
            if len(gc_bounds) > 2:
                errorText = GC_BOUNDS_ARG + ' must contain only up to two values'
                raise ValueError(errorText)
            continue
        if sys.argv[i] == OUTPUT_BASE_NAME_ARG:
            output_base_name = sys.argv[i + 1]
            i += 2
            continue

    print('\n ----->>  Your setup: \n')
    print(MIN_LENGTH_ARG + ' ' + str(min_length))
    print(KEEP_FILTERED_ARG, keep_filtered, sep=" ")
    print(GC_BOUNDS_ARG, gc_bounds, sep=" ")
    print(OUTPUT_BASE_NAME_ARG + ' ' + output_base_name)
    print('your input file name: ', input_file)
    print('\n ----->> Filtering: Start')

    fastq = open(input_file, 'r')
    passed_file = open(output_base_name + '__passed.fastq', 'w')
    if keep_filtered:
        failed_file = open(output_base_name + '__failed.fastq', 'w')

    lines = fastq.read().splitlines()

    count_passed = 0
    count_failed = 0


    passed, failed = filtrate(lines, min_length, gc_bounds)


    count_passed = len(passed)
    count_failed = len(failed)

    for fastq_read_info in passed:
        write_read_to_file(fastq_read_info, passed_file)

    if keep_filtered:
        for fastq_read_info in failed:
            write_read_to_file(fastq_read_info, failed_file)


    total_reads_number_str = str(len(lines) // 4)
    total_number_text = '\n ----->> total number of processed reads in '

    print(total_number_text + input_file + ': ' + total_reads_number_str)
    print('\n ----->> Filtering: Finish')
    print('\n ----->> number of passed reads: ' + str(count_passed))
    print('\n ----->> number of failed reads: ' + str(count_failed))

    percent = round(count_passed * 100 / (len(lines) // 4), 2)
    print('\n ----->> percent of passed reads: ' + str(percent))

    passed_file.close()
    fastq.close()
    if keep_filtered:
        failed_file.close()

if __name__ == '__main__':
    do()